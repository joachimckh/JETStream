...
public:
  ...
  torch::data::Example<> get(size_t index) override {
    ...
    for (const auto& label : labels) {
      compositions.push_back(jetstream::utils::getComposition(label));
    }


    // auto combinedData{torch::cat({ptTensor, etaTensor, phiTensor, energyTensor, massTensor, energyFractionTensor}, 0)};
    auto combinedData{torch::stack({ptTensor, etaTensor, phiTensor, energyTensor, massTensor}, 0)};
    auto target{torch::tensor(compositions, dataIntOpts)}; 
    return {combinedData, target};
  }
  ...
  torch::Tensor jetComposition(const auto &jet) {
    auto pt = jet.getConstituentPt();
    auto deltaR = jet.getDeltaR();
    auto jetIdx = jet.getJetIdx();
  }
