...
def train(model, dataloader, optimizer, config, writer, epoch):
  ...
  for ii, data in enumerate(tqdm(dataloader, desc=f"Epoch {epoch}")):
    optimizer.zero_grad()

    target = data["tar"].unsqueeze(0).float()
    pred = model(data)
    ...
    if writer is not None:
      global_step = epoch * len(dataloader) + ii
...
def test(model, dataloader, writer, epoch):
  ...
  with torch.no_grad():
    for data in dataloader:
      ...
      loss = criterion(pred, data["tar"].float())
      ...
      correct += pred.eq(data.tar.view_as(pred)).sum().item()
...
def main(yaml_path):
  ...
  for epoch in range(int(config.training.epochs)):
    _ = train(model, dataloader_train, optimizer, config, writer, epoch)
...
if __name__ == "__main__":
  parser = argparse.ArgumentParser(description='Train GNN to classify jet substructure')
